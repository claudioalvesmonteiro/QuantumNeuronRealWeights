...
def treinamentoNeuronio(operator, inputVector, weightVector, y_train, lrParameter = 0.09, threshold=None):
    ...
    if (operator == "hsgs"):
        ...
        resultado = executeNeuron(neuron, simulator, threshold=threshold)
        deltaRule(inputVector, weightVector, lr=lrParameter, y_train=y_train, out=resultado)
        return resultado
...
def experiment_TRAIN(Xs_train, ys_train, lrParameter=0.09, n_epochs=400, seed=1, threshold=None, trainingBias=True, trainingApproaches={}):
    ...
    for iteration in range(n_epochs):
        ...
        epoch_evolutionEncoding.append(erroEncoding)
    ...
    return bestWeightsEncoding, epoch_evolutionEncoding #bestWeightsHSGS, weightVectorsClassico, weightVectorsClassicoBin, erroEncoding

def experiment_TEST(Xs_test, ys_test, weightVectorsEncoding,
                    #weightVectorsHSGS, weightVectorsClassico, weightVectorsClassicoBin,  
                    threshold=0.5, lrParameter=0.1, repeat=30, bias=True, testingApproaches={}):
        ...
        for posicaoTreinamento in range(len(Xs_test)):
            ...
            for neuronClass in range(len(list(set(ys_test)))):
                ...
                if ("encoding-weight" in testingApproaches):
                    ...
                    resultadoEncoding1 = executeNeuron(neuron, simulator, threshold=threshold)
            ...
            outputsEncoding.append(neuronMaiorEncoding)
